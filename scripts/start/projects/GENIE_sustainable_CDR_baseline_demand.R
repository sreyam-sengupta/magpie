...
### Identifier and folder (same tree as GENIE_sustainable_CDR_baseline_price.R)
###############################################
...
# Food settings for baseline scenario (match price-driven baseline)
...
### Biodiversity module: same as baseline_price (bii_target, scalar BII = 0)
cfg$gms$biodiversity <- "bii_target"
cfg$gms$s44_cost_bii_missing <- 10000000
cfg$gms$c44_bii_decrease <- 1
cfg$gms$s44_bii_target <- 0

# No GHG price until loop overwrites (same pattern as other *_demand.R scripts)
...
beV <- c(0, 5, 7, 10, 15, 25, 45) #(0, 5, 7, 10, 15, 25, 45) must match price-driven BE levels used for readout
gV  <- c(0, 10, 20, 50, 100, 200, 400, 600, 1000, 2000, 3000, 4000) #(0, 10, 20, 50, 100, 200, 400, 600, 1000, 2000, 3000, 4000) GHG prices (MESSAGE-style grid)
mpV <- c(0)
...
for (mp in mpV) {
  ...
  for (be in beV) {
    ...
    # f60 column from Sustainable_CDR_baseline_rev4 price-driven readout (SSP2_old.tgz)
    cfg$gms$c60_2ndgen_biodem <- paste0("SSP2_Baseline_BD00_BE", be_str, "_G0000_price")
    ...
    for (g in gV) {
      ...
      cfg$title <- paste0(preflag, "_BE", be_str, "_G", g_str, "demand")
