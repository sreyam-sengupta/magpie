...
###############################################
identifierFlag <- "Sustainable_CDR_all_rev6"
cfg$title <- "Sustainable_CDR_all_rev6"
###############################################
...
beV <- c(0) # Options: 0, 5, 7, 10, 15, 25, 45

### GHG — US$2005 labels for run names; inflate CO2 via s56_cprice_red_factor
g_labelV <- c(400) # Options: 0, 10, 20, 50, 100, 200, 400, 600, 1000, 2000, 3000, 4000
...
for (mp in mpV) {
  ...
  for (be in beV) {
    ...
    cfg$gms$c60_2ndgen_biodem <- paste0("SSP2_all_BD78_BE", be_str, "_G0000_price_rev6")
