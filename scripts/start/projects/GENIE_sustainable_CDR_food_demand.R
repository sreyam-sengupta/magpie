...
###############################################
identifierFlag <- "Sustainable_CDR_food"
cfg$title <- "Sustainable_CDR_food"
###############################################
...
# Food switch: eliminate underweight
...
cfg$gms$s42_env_flow_scenario <- 0
cfg$gms$s42_env_flow_base_fraction <- 0.05
...
for (bl in blV) {
  ...
  for (mp in mpV) {
    ...
    for (be in beV) {
      ...
      # Demand input column: extracted from Sustainable_CDR_food price-driven runs
      cfg$gms$c60_2ndgen_biodem <- paste0("SSP2_Food_BD-none_BE", be_str, "_G0000price_rev3")
